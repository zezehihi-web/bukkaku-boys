...
ホームメイト（東建コーポレーション）の物件詳細ページから物件名・住所・賃料・面積・間取り・築年数を取得する。

実DOM構造（2026年3月確認）:
- サーバーサイドレンダリング（Playwright不要）
- ネストされたテーブル構造
- 建物ページに部屋一覧が載る形式
- span.m_prptydata_mon: 賃料
- th/td テーブルで各フィールド

URL形式: https://www.homemate.co.jp/rent/detail/{PropertyId}/
"""
...
async def parse_homemate_url(url: str) -> dict:
    """ホームメイトの物件詳細URLを解析して物件情報を抽出"""
    result = {
        "property_name": "",
        "address": "",
        "rent": "",
    ...
    if h1:
        ...
        # 「物件名 | ホームメイト」等を除去
        name = re.sub(r'\s*[|｜]\s*ホームメイト.*$', '', name)
        name = re.sub(r'の賃貸.*$', '', name)
        name = re.sub(r'の物件情報.*$', '', name)
        if name and not re.match(r'^(ホームメイト|東建)', name):
            result["property_name"] = name.strip()

    # --- 賃料 ---
    # span.m_prptydata_mon
    rent_el = soup.select_one("span.m_prptydata_mon")
    if rent_el:
