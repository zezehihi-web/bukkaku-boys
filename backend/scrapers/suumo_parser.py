...
SUUMOの物件詳細ページから物件名・住所・賃料・面積・間取りを取得する。
"""
...
async def parse_suumo_url(url: str) -> dict:
    ...
    # --- 物件名 ---
    # SUUMOの賃貸ページ: h1.section_h1-header-title
    for sel in [
        "h1.section_h1-header-title",
        "h1.property_view_title",
        ".property_view_title",
        "h1",
    ]:
    ...
    # SUUMOの賃料表示セレクタ
    if not result["rent"]:
        rent_el = soup.select_one(".property_view_note-emphasis")
        if rent_el:
    ...
    for key in ["間取り"]:
        if key in table_data:
            ...
            break
    # 間取りがテーブルにない場合、ページ全体から探す
    if not result["layout"]:
        layout_el = soup.select_one(".property_view_table-body--madori")
        if layout_el:
            result["layout"] = layout_el.get_text(strip=True)
